...
void Transfer::preinit() {
    ...
    for(uint32_t i = 0; i<pools.size(); i++) {
        pools[i] = win->device.logical.createCommandPool(vk::CommandPoolCreateInfo(vk::CommandPoolCreateFlagBits::eTransient, win->device.t_i));
    }
...
void Transfer::render(uint32_t i) {
    ...
    if(fence_num > 0) {
        win->device.logical.waitForFences(fence_num, fences.data(), true, 10000000000L);
        fence_num = 0;
...
void Transfer::record(std::function<void(const vk::CommandBuffer&)> f, foNode* waiter, vk::PipelineStageFlags stage) {
